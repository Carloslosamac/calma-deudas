...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // El wall-clock real del edge function ronda los 150s. Los timeouts por
    // fase deben CABER dentro del presupuesto: si no, el proceso muere antes
    // de cerrar el run y este queda huérfano en "running" hasta que el
    // saneamiento lo marca como fallo (de ahí las duraciones de 44 min).
    const TIME_BUDGET_MS = 135_000;
    const ARTICLE_TIMEOUT_MS = 105_000;
    const HERO_TIMEOUT_MS = 30_000;
    ...
    for (const row of batch) {
      ...
      // Necesitamos margen para artículo + portada + cierre del run. Si no
      // cabe un post entero, paramos aquí en vez de morir a mitad.
      // El primer post siempre se intenta; a partir del segundo exigimos
      // margen suficiente para artículo + portada + cierre del run.
      if (published.length + failed.length > 0 && remaining() < 60_000) {
      ...
      try {
        article = await withTimeout(
          generateArticle(row),
          Math.min(ARTICLE_TIMEOUT_MS, Math.max(10_000, remaining())),
          `generateArticle(${row.id})`,
        );
      } catch (e) {
    ...
    if (runId) {
      await supabase
        ...
        .update({
          status:
            failed.length && published.length === 0 && !stoppedByBudget
              ? "failed"
              : "success",
          finished_at: new Date().toISOString(),
          target,
          ...
          failed_count: failed.length,
          error: failed.length
            ? `[${PIPELINE_VERSION}] Fallaron roadmap ids: ${failed.join(", ")}${stoppedByBudget ? " (parado por presupuesto de tiempo)" : ""}`
            : stoppedByBudget
              ? `[${PIPELINE_VERSION}] Parado por presupuesto de tiempo antes de completar el batch`
              : null,
        })
        .eq("id", runId);
    }
